...
#current/Rakefile admin:crawl_pages URL=http://www.aspendemo.com SITEMAP=true  #Sitemap builder for aspendemo
...
class Crawler
  ...
  def extract_and_call_urls(url)            
    ...
    return if page.instance_of?(Mechanize::File) || page.content_type.index('text/html') == nil
	...
	def update_google
	    ...
	    begin
  	    Net::HTTP.get('www.google.com',
  	                  ...
  	                  escaped_sitemap_uri) unless @debug
        puts "Notified Google"
      rescue
        puts "Problems contacting google"
      end
      #Ask.com
	    begin
	      Net::HTTP.get('submissions.ask.com',
	                  'ping?sitemap=' +
	                  escaped_sitemap_uri) unless @debug
        puts "Notified Ask"
      rescue
        puts "Problems contacting Ask"
      end
      #Yahoo
	    begin
        Net::HTTP.get('search.yahooapis.com',
	                  '/SiteExplorerService/V1/updateNotification?appid=YahooDemo&url' +
	                  escaped_sitemap_uri) unless @debug
        puts "Notified Yahoo"
      rescue
        puts "Problems contacting Yahoo"
      end
